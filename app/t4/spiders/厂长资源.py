...
class Spider(BaseSpider):  # 元类 默认的元类 type
    ...
    def getCookie(self, url):
        ...
        if '人机验证' in rsp.text:
            ...
            session.get(
                'https://czzy.top/a20be899_96a6_40b2_88ba_32f1f75f1552_yanzheng_ip.php?type=96c4e20a0e951f471d32dae103e83881&key={0}&value={1}'.format(
                    key, value), headers=header)
            return session.get(url, headers=header)
        elif '检测中' in rsp.text:
            append = self.regStr(rsp.text, 'href =\"(/.*?)\"')
            session.get('https://czzy.top{0}'.format(append), headers=header)
            return session.get(url, headers=header)
        else:
            return rsp
    ...
    def searchContent(self, wd, quick=False, pg=1):
        url = 'https://czzy.top/daoyongjiekoshibushiyoubing?q={0}'.format(urllib.parse.quote(wd))
        rsp = self.getCookie(url)
        root = self.html(self.cleanText(rsp.text))
